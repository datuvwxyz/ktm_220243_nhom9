    ...
    <form method="post" action="{{route('product.update',$product->id)}}">
      ...
      <div class="form-group">
        ...
        <select name="cat_id" id="cat_id" class="form-control">
          <option value="">--Chọn danh mục bất kì--</option>
      ...
      <div class="form-group {{(($product->child_cat_id)? '' : 'd-none')}}" id="child_cat_div">
        ...
        <select name="child_cat_id" id="child_cat_id" class="form-control">
          <option value="">--Chọn danh mục con bất kì--</option>
      ...
      <div class="form-group">
        ...
        <select name="size[]" class="form-control selectpicker" multiple data-live-search="true">
          <option value="">--Chọn kích thước bất kì--</option>
          @foreach($items as $item)
          @php
          $data=explode(',',$item->size);
          // dd($data);
          @endphp
          <optgroup label="Men (Nam)">
            @foreach([39, 40, 41, 42, 43, 44, 45] as $size)
            <option value="{{ $size }}" @if(in_array((string)$size, $data)) selected @endif>{{ $size }}</option>
            @endforeach
          </optgroup>
          <optgroup label="Women (Nữ)">
            @foreach([35, 36, 37, 38, 39, 40, 41] as $size)
            <option value="{{ $size }}" @if(in_array((string)$size, $data)) selected @endif>{{ $size }}</option>
            @endforeach
          </optgroup>
          <optgroup label="Kids (Trẻ em)">
            @foreach([27, 28, 29, 30, 31, 32, 33, 34] as $size)
            <option value="{{ $size }}" @if(in_array((string)$size, $data)) selected @endif>{{ $size }}</option>
            @endforeach
          </optgroup>
          @endforeach
      ...
      <div class="form-group">
        ...
        <select name="brand_id" class="form-control">
          <option value="">--Chọn thương hiệu bất kì--</option>
      ...
      <div class="form-group">
        ...
        <select name="condition" class="form-control">
          <option value="">--Chọn tình trạng bất kì--</option>
